...
class TestIntegration(unittest.TestCase):
    ...
    def render_demo_images(self, renderer, cleanup=True):
    ...
    def test_integration(self):
        _, vtk_image_sets = self.render_demo_images("vtk")
        _, client_image_sets = self.render_demo_images("client")
    ...
    def test_gltf_conversion(self):
        result, _ = self.render_demo_images("client", False)

        gltf_file_path = None
        pattern = r"RenderEngineGltfClient: .* scene exported to '(.*)'"
        for line in result.stdout.splitlines():
            match = re.search(pattern, line)
            if match:
                (gltf_file_path,) = match.groups()
                break
